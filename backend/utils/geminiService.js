...
//     const prompt = `Provide a concise summary of the following text, highlighting the key concepts, main ideas , and Keep the summary clear and structred.
...
// //     const prompt = `Based on the following context from a document , Analyze the context and answer the user's questions .If the answer is not in the context , say no.
        ...
        for (const block of blocks) {
            ...
            for (const line of lines) {
                ...
                if (trimmed.startsWith("Q:"))
                    ...
                    const match = trimmed.match(/O([1-4])/);
                    if (match) correctIndex = parseInt(match[1]) - 1;
                }
...
export const generateSummary = async (text) => {
    const prompt = `
Provide a concise and structured summary highlighting:
- Key concepts
- Main ideas
...
export const explainConcept = async (concept, context) => {
    const prompt = `
Explain the concept of "${concept}" clearly and simply.
