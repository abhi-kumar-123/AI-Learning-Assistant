            ...
            <div className='mt-2 pt-4 border-t border-slate-100 '>
                {quiz?.userAnswers?.length > 0 ? (
                    <Link to={`quizzes/${quiz._id}/results`} >
                        <button
                            className='group/btn w-full inline-flex items-center justify-center gap-2 h-11 bg-slate-100 hover:bg-slate-200 text-sm text-slate-700 font-medium rounded-xl transition-all duration-200 active:scale-95 cursor-pointer '
                        >
                    ...
                    <Link to={`quizzes/${quiz._id}`} >
                        <button
                            className='group/btn w-full h-11  bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-lg transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 overflow-hidden '
                        >
